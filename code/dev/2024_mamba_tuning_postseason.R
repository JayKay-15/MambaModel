...
nba_final <- tbl(dbConnect(SQLite(), "../nba_sql_db/nba_db"), "mamba_long_odds_post") %>%
    ...
    select(
        season_year:opp_is_b2b_second, series_odds:opp_implied_prob,
        win_pct_vs_post:opp_series_wins, team_fgm:opp_opp_pct_uast_fgm
    ) %>% na.exclude()

nba_final_train <- nba_final %>%
    filter(season_year < 2024 & location == 1) %>%
    select(game_id, team_winner,
           series_odds, over_under, team_implied_prob,
           win_pct_vs_post:opp_series_wins,
...
nba_final_win_outputs <- nba_final %>%
    filter(season_year == 2024 & location == 1) %>%
    select(season_year:opp_game_count_season, series_odds:opp_series_wins)
...
sampled_game_ids <- sample(unique(nba_final_cor$game_id),
                           ...
                           replace = FALSE) # dataset is too small for this
...
cor_df <- nba_cor %>%
    select(team_fgm:team_opp_pct_uast_fgm) %>%
    select(-contains("_rating"))
