...
team_logs <- nbastatR::game_logs(seasons = 2022, result_types = "team")
...
play_logs_all <- play_by_play_v2(game_ids = unique(games$idGame[1:11]))
...
others_qtr <- new_pbp %>% # mine
  ...
  mutate(team_player = case_when(player_number == "player1_name" ~ player1_team_abbreviation,
                                 ...
                                 player_number == "player3_name" ~ player2_team_abbreviation,
                                 TRUE ~ "None")) %>%
